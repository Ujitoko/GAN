...
class DTD:
    def __init__(self):
        print("init DTD")
        with open("./dtd/labels/labels_joint_anno.txt", "r") as file:
            data = file.read()
    ...
    def extract(self, num, size):
        ...
        for (i, tex) in enumerate(ex_texture):
            ...
            """
            if len(tex_list[0]) < 2:
                rand_index = np.random.randint(0, self.texture_len, size=1)
                tex_tmp = self.texture[rand_index]
                tex_list = tex_tmp[]
            """
            img = Image.open(os.path.join("./dtd/images", tex_list[0]))
            img_np = np.array(img)

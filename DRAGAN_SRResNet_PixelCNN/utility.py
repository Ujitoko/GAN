...
class Pokemon:
    def __init__(self):
        self.path = "./pokemon"
        self.npy_files = np.array(os.listdir(self.path))
        print(len(self.npy_files))
    def extract(self, num, size):
        rand_index = np.random.randint(0, len(self.npy_files)-1, size=num)
        #print(rand_index)

        imgs_np = np.empty((0, size, size, 3), np.float32)
        for file in self.npy_files[rand_index]:

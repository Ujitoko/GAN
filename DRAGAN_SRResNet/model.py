...
def conv2d_layer(input_layer, output_dim, kernel_size = 3, strides = 2, stddev = 0.02, name = 'conv2d'):
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(input_layer, weight, [1, strides, strides, 1], padding = 'SAME')
        conv = tf.nn.bias_add(conv, bias)
        return conv

def deconv2d_layer(input_layer, output_shape, kernel_size = 2, strides=2, stddev = 0.02, name = 'deconv'):
    with tf.variable_scope(name):
        init_weight = tf.truncated_normal_initializer(mean = 0.0, stddev = stddev, dtype = tf.float32)
        ...
        bias = tf.get_variable(
            ...
            initializer = tf.constant_initializer(0.0))
        deconv = tf.nn.conv2d_transpose(input_layer, weight, output_shape, strides = [1, strides, strides, 1], padding = 'SAME')
